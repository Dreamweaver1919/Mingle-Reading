...
`scripts/` now contains two entrypoints:

- `scripts/source_registry_manifest_builder.py`
...
- `scripts/hierarchical_dataset_builder.py`
...
Use this when a local source should first be registered into the `data/`-style directory layout and manifest structure.
...
```bash
python scripts/source_registry_manifest_builder.py scripts/demo_book.txt --mode books
```

This writes to `scripts/registry_output/` by default:

```text
scripts/registry_output/
  data/
    raw/
      books/
...
```bash
python scripts/source_registry_manifest_builder.py path/to/persona_notes.txt --mode persona_sources --persona-name "Lu Xun" --source-type author_work
```
...
- `--output-root scripts/registry_output_custom`
- `--version v002`
...
- Raw source filenames follow the `docs/data/muse_reading_data_design.md` pattern for books:
  - `book_<title_slug>__source_<source_type>__v001.json`
- Source manifests are stored under `data/manifests/` and batch registry files are grouped by mode:
...
```bash
python scripts/hierarchical_dataset_builder.py scripts/demo_book.txt --output-dir scripts/build_output/demo_run
```
