...
def _raw_record_from_txt(path: Path, title: str | None) -> dict[str, Any]:
    ...
    return {
        ...
        "content": content,
        "notes": "Generated from plain text input by scripts/hierarchical_dataset_builder.py",
    }
...
def _compute_salience(text: str, chapter_index: int) -> dict[str, float]:
    ...
    conflict_markers = len(
        re.findall(
            r"(fight|war|anger|fear|death|secret|against|refuse|conflict|argue|betray|哭|怒|恨|怕|死|秘密|争|吵|拒)",
            text,
            flags=re.IGNORECASE,
        )
    )
    abstract_markers = len(
        re.findall(
            r"(memory|freedom|truth|fate|history|dream|identity|meaning|symbol|命运|记忆|自由|真相|历史|理想|象征)",
            text,
            flags=re.IGNORECASE,
        )
    )
...
def _paragraph_tags(text: str) -> list[str]:
    ...
    if re.search(r"(memory|remember|past|回忆|记得|往事)", lowered):
        tags.append("memory")
    if re.search(r"(dream|future|命运|理想|未来)", lowered):
        tags.append("foreshadowing")
    if re.search(r"(fight|argue|war|anger|争|吵|怒)", lowered):
