...
- `highlight_qa/demo/*.jsonl`
  - line-delimited samples aligned with `schemas/highlight_qa.schema.json`
- `anti_spoiler/demo/*.jsonl`
  - line-delimited samples aligned with `schemas/anti_spoiler_eval.schema.json`
- `chapter_summary/demo/*.jsonl`
  - lightweight summary checks used by `eval/run_eval.py`

## Scope

These fixtures are intentionally tiny and synthetic:

- they target the bundled `examples/muse_demo_book.txt`
